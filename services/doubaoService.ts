...
export const generateShotList = async (
  ...
  const processScene = async (
    ...
    const prompt = `
      ...
      - shotSize（字符串类型）
      - characters（字符串数组类型）
      - keyframes（对象数组类型，对象包含 id、type（取值为 ["start", "end"]）、visualPrompt（中文描述） 字段）
...
export const generateVideo = async (
  ...
  const endpoint = `${DOUBAO_CONFIG.API_ENDPOINT}/contents/generations/tasks`;
...
const pollVideoTask = async (taskId: string): Promise<string> => {
  const endpoint = `${DOUBAO_CONFIG.API_ENDPOINT}/contents/generations/tasks/${taskId}`;
